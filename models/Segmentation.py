...
from .geometry_processing_at_RBF import dMaSIFConv, tangent_vectors
from .geometry_processing_at_RBF import diagonal_ranges
...
class Conv(torch.nn.Module):
    ...
    def load_mesh(self, xyz, triangles=None, normals=None, nuv=None, weights=None, batch=None):
        ...
        if nuv ==None and weights!=None:
            ...
            # rho2_ij = ((x_j - x_i) ** 2).sum(-1)    # (N, N, 1)
            # Gaussian window:
            alpha = 3.0
            window_ij = (1.0 + rho2_ij / alpha) ** (-alpha)
