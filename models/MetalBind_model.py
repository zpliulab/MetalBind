import torch
import os, sys
from .Segmentation import Conv
from torch import nn
import numpy as np
import time
from torch_scatter import scatter_max, scatter_add
import torch.nn.functional as F

def printf(args, *content):
    file = sys.stdout
    f_handler = open(os.path.join(args.checkpoints_dir, 'log.txt'), 'a+')
    sys.stdout = f_handler
    print(' '.join(content))
    f_handler.close()
    sys.stdout = file
    print(' '.join(content))

class EarlyStopping:
    def __init__(self, opt, patience_stop=10, patience_lr=5, verbose=False, delta=0.0001, path='check1.pth'):
        self.opt = opt
        self.stop_patience = patience_stop
        self.lr_patience = patience_lr
        self.verbose = verbose
        self.counter = 0
        self.best_score = None
        self.early_stop = False
        self.val_loss_min = np.Inf
        self.delta = delta
        self.path = path

    def __call__(self, val_auc, model):
        if self.best_score is None:
            self.best_score = val_auc
            self.save_checkpoint(model)
            printf(self.opt, 'saving best model...')
        elif val_auc <= self.best_score + self.delta:
            self.counter += 1
            if self.counter == self.lr_patience:
                self.adjust_lr(model)
                # self.counter = 0

            if self.counter >= self.stop_patience:
                self.early_stop = True
        else:
            self.best_score = val_auc
            self.save_checkpoint(model)
            self.counter = 0
            printf(self.opt, 'saving best model...')

    def adjust_lr(self, model):
        lr = model.optimizer.param_groups[0]['lr']
        lr = lr/10
        for param_group in model.optimizer.param_groups:
            param_group['lr'] = lr
        model.load_state_dict(torch.load(self.path))
        printf(self.opt, 'loading best model, changing learning rate to %.7f' % lr)

    def save_checkpoint(self, model):
        torch.save(model.state_dict(), self.path)


class SubspaceAttention(nn.Module):
    """
    Subspace Attention
    """
    def __init__(self, embed_dim, num_heads, subspaces=4, dropout=0.1):
        super().__init__()
        assert embed_dim % subspaces == 0, "embed_dim must be divisible by subspaces"
        self.S = subspaces
        self.sub_dim = embed_dim // subspaces
        self.attns = nn.ModuleList([
            nn.MultiheadAttention(
                embed_dim=self.sub_dim,
                num_heads=min(num_heads, self.sub_dim),
                dropout=dropout,
                batch_first=True
            ) for _ in range(self.S)
        ])

    def forward(self, x):
        # x: (B, M, E)
        splits = x.split(self.sub_dim, dim=-1)  # List[S] of (B, M, sub_dim)
        outputs = []
        for i, attn in enumerate(self.attns):
            out_i, _ = attn(splits[i], splits[i], splits[i])
            outputs.append(out_i)
        # 拼接回 (B, M, E)
        fused = torch.cat(outputs, dim=-1)
        return fused


class MetalBind(nn.Module):
    """ Class for training Model weights

    :args opt: structure containing configuration params
    e.g.,
    --dataset_mode -> classification / segmentation)
    --arch -> network type
    """
    def __init__(self, opt):
        super(MetalBind, self).__init__()
        self.opt = opt

        feature_type_dim = {'esm':1152, 'chemi':6, 'pka':3}
        in_channels = 0
        for item in self.opt.input_feature_type:
            in_channels += feature_type_dim[item]
        self.gpu_ids = opt.device
        self.device = torch.device('{}'.format(self.gpu_ids)) if self.gpu_ids else torch.device('cpu')

        I = in_channels
        E = opt.emb_dims
        H = opt.post_units


        self.modality_proj = nn.ModuleDict({
            f: nn.Linear(feature_type_dim[f], E)
            for f in opt.input_feature_type
        }).to(self.device)


        num_heads = getattr(opt, 'num_heads', 8)
        subspaces = getattr(opt, 'attn_subspaces', 4)
        dropout = getattr(opt, 'dropout', 0.0)
        self.modality_attn = SubspaceAttention(
            embed_dim=E,
            num_heads=num_heads,
            subspaces=subspaces,
            dropout=dropout
        ).to(self.device)


        self.lr = opt.lr
        self.nclasses = opt.nclasses

        self.conv = Conv(
            opt,
            in_channels=E,
            out_channels=E,
            n_layers=opt.n_layers,
            radius=opt.radius,
        ).to(self.device)


        self.net_out=nn.Sequential(
            nn.Linear(E, H),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Linear(H, H),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Linear(H, 2),
        ).to(self.device)

        self.optimizer = torch.optim.Adam(self.parameters(), lr=self.lr, amsgrad=True)
        if self.opt.loss_type == 'interface':
            self.criterion = torch.nn.CrossEntropyLoss().to(self.device)
        else:
            self.criterion = torch.nn.NLLLoss().to(self.device)

    def set_input(self, data):
        for k, v in data.items():
            if isinstance(v, torch.Tensor):
                data[k] = v.to(self.device)  # ← contentReference[oaicite:9]{index=9}

        # 2)
        alt_map = {
            'esm': ['hmm', 'seq', 'seq_feat'],
            'pka': ['geo', 'chem', 'pka_feat'],
            'chemi': ['chem', 'chemi_feat'],
        }

        def _get_field(data_obj, key, alt_list=None):
            alt_list = alt_list or []
            # 1) dict-like
            if isinstance(data_obj, dict) and key in data_obj:
                return data_obj[key]
            # 2) attribute
            if hasattr(data_obj, key):
                return getattr(data_obj, key)
            # 3) __getitem__
            try:
                return data_obj[key]
            except Exception:
                pass
            # 4) try alternatives
            for alt in alt_list:
                if isinstance(data_obj, dict) and alt in data_obj:
                    return data_obj[alt]
                if hasattr(data_obj, alt):
                    return getattr(data_obj, alt)
                try:
                    return data_obj[alt]
                except Exception:
                    pass
            raise KeyError(f"Field '{key}' not found in data. Tried alternatives: {alt_list}")

        proj_feats = []
        for f in self.opt.input_feature_type:
            try:
                feat_tensor = _get_field(data, f, alt_map.get(f, []))
            except KeyError as e:
                available = None
                try:
                    if isinstance(data, dict):
                        available = list(data.keys())
                    else:
                        available = [k for k in dir(data) if not k.startswith('_')]
                except Exception:
                    available = 'unknown'
                raise KeyError(f"{e}\nAvailable keys/attrs in data: {available}")
            proj_feats.append(self.modality_proj[f](feat_tensor))

        # proj_feats = [self.modality_proj[f](data[f]) for f in self.opt.input_feature_type]

        # 3)
        stacked = torch.stack(proj_feats, dim=1)
        attn_out = self.modality_attn(stacked)
        fused = attn_out.mean(dim=1)

        # 4) embed / optimize / test
        self.P = {
            'input_features': fused,
            'xyz':        data['xyz'],
            'nuv':        data['nuv'],
            'batch':      data['xyz_batch'],
            'y':          data['y'].long(),
            'site':       data['site'].long(),
            'site_point': data['site_point'].long(),
        }

    def embed(self):
        torch.cuda.synchronize(device=self.device)
        torch.cuda.reset_max_memory_allocated(device=self.device)
        begin = time.time()
        self.conv.load_mesh(
            self.P["xyz"],
            triangles=None,
            nuv=self.P["nuv"],
            batch=self.P["batch"],
        )

        self.P["embedding"] = self.conv(self.P['input_features'])
        # print("self.P['embedding]1:", self.P["embedding"])
        # print("self.P['input_features']1:", self.P['input_features'])
        # print("Contains NaN?", torch.isnan(self.P['input_features']).sum().item())
        torch.cuda.synchronize(device=self.device)
        end = time.time()
        memory_usage = torch.cuda.max_memory_allocated(device=self.device)
        conv_time = end - begin
        return conv_time, memory_usage

    def optimize_parameters(self):
        self.train()
        conv_time, memory_usage = self.embed()
        self.P["iface_preds"] = self.net_out(self.P["embedding"]).squeeze()
        self.optimizer.zero_grad()
        self.loss = self.compute_loss()
        self.loss.backward()
        self.optimizer.step()
        if self.opt.loss_type == 'interface':
            return torch.softmax(self.P["iface_preds"], dim=1)[:, 1]
        else:
            iface_softmax = torch.softmax(self.P["iface_preds"], dim=1)[:, 1]
            return scatter_max(iface_softmax, self.P['site_point'], dim=0)[0]

    def test(self):
        """tests model
        returns the likelihood of sites or interfaces be binding
        """
        self.eval()
        with torch.no_grad():
            conv_time, memory_usage = self.embed()
            self.P["iface_preds"] = self.net_out(self.P["embedding"]).squeeze()
            self.loss = self.compute_loss()
        if self.opt.loss_type == 'interface':
            return torch.softmax(self.P["iface_preds"], dim=1)[:, 1]
        else:
            try:
                iface_softmax = torch.softmax(self.P["iface_preds"], dim=1)[:, 1]
            except:
                print('174')
            return scatter_max(iface_softmax, self.P['site_point'], dim=0)[0]

    def compute_loss(self):
        if self.opt.loss_type == 'interface':
            loss = self.criterion(self.P["iface_preds"], self.P['y'])
        else:
            _, pos = scatter_max(torch.softmax(self.P["iface_preds"], dim=1), self.P['site_point'], dim=0)
            pos = pos[:,1]
            self.preds_sites = torch.softmax(self.P["iface_preds"], dim=1)[pos]
            loss = self.criterion(torch.log(self.preds_sites), self.P['site'])
        return loss

    def load_network(self, which_epoch='best'):
        save_filename = '%s.pth' % which_epoch
        save_path = os.path.join(self.opt.checkpoints_dir, save_filename)

        state = torch.load(save_path, map_location=self.device)
        state = {
            k.replace('modality_proj.hmm.', 'modality_proj.esm.').replace('modality_proj.geo.', 'modality_proj.pka.'): v
            for k, v in state.items()}
        self.load_state_dict(state)
        printf(self.opt, 'best model loaded:', save_path)

    def save_network(self, which_epoch):
        """save model to disk"""
        save_filename = '%s_net.pth' % which_epoch
        save_path = os.path.join(self.opt.checkpoints_dir, save_filename)
        if torch.cuda.is_available():
            torch.save(self.cpu().state_dict(), save_path)
            self.cuda(self.device)
        else:
            torch.save(self.cpu().state_dict(), save_path)
